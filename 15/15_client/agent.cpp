...
void Agent::parseRequest (QString req)
{
    ...
    qDebug() << "пришло: " << request;
    if (request.contains("start")){
        startScan();
    }
    if (request.contains("update")){
        QString tx = request["update"].toString();
        qDebug() << tx;
        starread(tx);
    }
    if (request.contains("vi")){
        QString tx = request["vi"].toString();
        bred(tx);
    }
}
...
void Agent::starread(QString lol)
{
    Parser *p = new Parser ();
    p->oche = lol;
    QThread *q = new QThread ();
    p->moveToThread (q);
    connect (q, &QThread::started, p, &Parser::upd);
    connect (p, &Parser::getResult, this, &Agent::slotGetResult);
    connect (q, &QThread::finished, p, &Parser::deleteLater);
    connect (q, &QThread::finished, q, &QThread::deleteLater);
    q->start();
...
void Agent::bred(QString lol)
{
    Parser *p = new Parser ();
    p->bbb = lol;
    QThread *q = new QThread ();
    p->moveToThread (q);
    connect (q, &QThread::started, p, &Parser::bu);
    connect (p, &Parser::getResult, this, &Agent::slotGetResult);
    connect (q, &QThread::finished, p, &Parser::deleteLater);
    connect (q, &QThread::finished, q, &QThread::deleteLater);
    q->start();
